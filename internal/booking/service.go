...
func (r *Service) Book(b Booking) (Booking, error) {
	return r.store.Book(b)
}
...
func (s *Service) ReleaseSeat(ctx context.Context, bookingID string, userID string) error {
	return s.store.Release(ctx, bookingID, userID)
}
